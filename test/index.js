...
diorama.registerScenario("create user", async (s, t, { alice }) => {
  // Make a call to a Zome function
  // indicating the function, and passing it an input
  const anchor_addr = await alice.call("my_zome", "create_anchor", {})
  console.log('anchor_addr', anchor_addr)
  const user_addr = await alice.call("my_zome", "create_user", {"user" : {"name":"Joe"}})
  console.log('user_addr', user_addr)
  const result = await alice.call("my_zome", "get_users", {})

  // check for equality of the actual and expected results
  t.deepEqual(result, { Ok: { name: 'users', items: [ { name: 'John Doe' } ] } })
})
